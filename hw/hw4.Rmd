---
title: "PASPE 2024: R básico"
subtitle: "Tarea 4"
author: "Rossana Torres Alvarez"
date: "`r format(Sys.time(), '%B %d, %Y')`"
output:
  word_document: default
---


```{r setup, include=FALSE}
library(tidyverse)
knitr::opts_chunk$set(echo = TRUE)
```

# Problema 1. Funciones e if else statements

1. Explica lo que hace esta función y muestra un ejemplo de su uso

```{r, eval = TRUE}
f3 <- function(x, y) {
  rep(y, length.out = length(x))
}
```


```{r, echo=TRUE}
# La funcion repite el elemento y, tantas veces como la longitud del objeto x. Demostracion:

# x es un vector de 3 elementos
x <- c(1:3)
# Verifico
length(x)

# y es una variable numerica con valor 1 
y = 1

# Mi funcion me debe imprimir 1 tres veces:
f3(x = c(1:3), y = 1)

```


2. Escribe una función llamada vectorType, que tenga como input un vector y devuelva la frase: "El vector es numerico" o "El vector es caracter" o "El vector no es ni numerico ni caracter" (Hint: Usa las funciones is.numeric(), is.character())

```{r, echo=TRUE}
vectorType = function(vector){
  if(is.numeric(vector)) {
    print("El vector es numerico")
  } else if (is.character(vector)){
    print("El vector es caracter")
  } else {
    print("El vector no es ni numerico ni caracter")
  }
}



```
3. Prueba tu función con los siguientes vectores
```{r, warning=FALSE}
x = 1:10
y = c("Copito", "Firulais")
z = c(TRUE, FALSE, NA)

```
3. Explica la diferencia entre un if else statement y la función ifelse() que usamos en el laboratorio 2


```{r, echo = TRUE, eval=FALSE}
# La condición dentro de un if else statement nos devuelve un solo valor logico. En cambio, la condición dentro de la funcion ifelse nos devuelve vectores de valores logicos. Por lo tanto, la funcion ifelse nos permite aplicar tests logicos a vectores, es decir, esta funcion esta vectorizada. 


```

## Problema 2. For loops

1. Escribir un for loop para calcular la media de cada columna en el dataframe mtcars (precargada).

```{r, warning=FALSE, echo=TRUE, eval=TRUE}
medias <- c()
print(medias)

for (col in 1:ncol(mtcars)) {

 medias[col] <- (mean(mtcars[,col]))
  
}

# Verificar
print(medias)
```


2. Instala el paquete nycflights13. Al cargar el paquete, tendrás disponible una base de datos llamada flights. Verifica qué clase de objeto es la base flights. Si es necesario, conviértela a data.frame. Determina el tipo de cada columna en la base usando un for loop. Si la consola te da un warning lee con cuidado y verifica tu resultado.

```{r, warning=TRUE, echo=TRUE, eval=TRUE}
#install.packages("nycflights13") # Esto solo se hace una vez en la vida!!
library(nycflights13)

# Inspecciono
class(flights)

# Cambio a dataframe
flights_df <- data.frame(flights)

tipo <- c()

print(tipo)

for (col in 1:ncol(flights_df)) {

 tipo[col] <- (class(flights_df[,col]))[1]
  
}

# Verificar
print(tipo)
```


3. Escribir un for loop que calcule el número de valores únicos en cada columna del dataframe iris (precargada en R). Si la consola te da un warning lee con cuidado y verifica tu resultado.
```{r, warning=TRUE, eval = TRUE, echo=TRUE}


unicos <- c()

print(unicos)

for (col in 1:ncol(iris)) {

 unicos[col] <- length(unique(iris[,col]))
  
}

# Verificar
print(unicos)
```

