...
Antropometria$talla_unlabelled <- as.numeric(Antropometria$talla)
...
# Opcion 1 para estimar cuantiles 
...
quantile(Antropometria$talla_unlabelled, na.rm = TRUE)

# Opcion 1 para estimar el maximo y el minimo:
#Uso la funcion max y min para estimar el maximo y minimo de talla
max(Antropometria$talla_unlabelled, na.rm = TRUE)

min(Antropometria$talla_unlabelled, na.rm = TRUE)

# Opcion 2 uso la funcion summary para obtener un resumen de las estadisticas
summary(Antropometria$talla)
# Mismo resultado si uso mi nueva variable sin etiquetas
summary(Antropometria$talla_unlabelled)
```

## Problema 2. Manejo de datos
1. Crea una nueva variable llamada id, que junte las variables folio e intp

**HINT: Usa la funcion paste0**

2. Usa la funcion unique() para *contar* el numero de ids unicos.

**HINT: Usa primero la funcion unique y despues la funcion length**
...
# Con pipes (equivalente a lo de arriba)
